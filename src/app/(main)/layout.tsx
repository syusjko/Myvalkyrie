      ...
      <main style={{ flex: 1, maxWidth: '100%', width: '100%', margin: '0', padding: '1.5rem 2rem', display: 'grid', gridTemplateColumns: isRightSidebarOpen ? '280px 1fr 380px' : '280px 1fr 60px', gap: '2rem', transition: 'grid-template-columns 0.3s ease' }}>
        ...
        <div style={{ position: 'sticky', top: '1.5rem', height: 'calc(100vh - 3rem)', minHeight: 0, display: 'flex', flexDirection: 'column', gap: '1.5rem', overflowY: 'auto', paddingRight: '10px' }} className="hidden-scrollbar">
          ...
          <Link href="/consensus" style={{ textDecoration: 'none' }}>
            <div style={{ padding: '1rem', background: 'var(--surface-color)', border: '1px solid var(--glass-border)', borderRadius: '12px', display: 'flex', alignItems: 'center', gap: '12px', color: 'var(--text-primary)', fontWeight: 'bold', fontSize: '1.1rem', transition: 'all 0.2s', cursor: 'pointer' }} onMouseOver={e => e.currentTarget.style.borderColor = '#10b981'} onMouseOut={e => e.currentTarget.style.borderColor = 'var(--glass-border)'}>
              <div style={{ background: '#10b981', width: '32px', height: '32px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>
          ...
          <div className="glass-panel" style={{ padding: '0', display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
